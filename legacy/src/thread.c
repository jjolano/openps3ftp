...
void threadpool_dispatch(struct ThreadPool* pool, void (*func)(void*), void* arg)
{
	if(pool != NULL)
	{
		if(sys_thread_mutex_lock(pool->mutex) == 0)
		{
			...
			{
				/* The legacy pool races: a worker can take the job and
				 * signal cond_get before this thread enters the wait,
				 * losing the wakeup. Broadcast + timed wait re-checks
				 * close the window. */
				sys_thread_cond_broadcast(pool->cond_job);
				sys_thread_cond_timedwait(pool->cond_get, pool->mutex, 50);
			}

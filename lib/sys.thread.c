...
#ifdef CELL_SDK
inline void* sys_mutex_alloc(int num)
{
	return malloc(sizeof(sys_lwmutex_t) * num);
}

inline int sys_mutex_create(void* ptr_mutex)
{
...
inline int sys_mutex_lock(void* ptr_mutex)
{
	return sys_lwmutex_lock((sys_lwmutex_t*) ptr_mutex, 0);
}

inline int sys_mutex_trylock(void* ptr_mutex)
{
	return sys_lwmutex_trylock((sys_lwmutex_t*) ptr_mutex);
}

inline int sys_mutex_unlock(void* ptr_mutex)
{
	return sys_lwmutex_unlock((sys_lwmutex_t*) ptr_mutex);
}

inline int sys_mutex_destroy(void* ptr_mutex)
{
	return sys_lwmutex_destroy((sys_lwmutex_t*) ptr_mutex);
}

inline void sys_mutex_free(void* ptr_mutex)
{
	free((sys_lwmutex_t*) ptr_mutex);
}

inline void* sys_cond_alloc(int num)
{
	return malloc(sizeof(sys_lwcond_t) * num);
}

inline int sys_cond_create(void* ptr_cond, void* ptr_mutex)
{
...
inline int sys_cond_wait(void* ptr_cond, void* ptr_mutex)
{
	return sys_lwcond_wait((sys_lwcond_t*) ptr_cond, 0);
}

inline int sys_cond_signal(void* ptr_cond)
{
	return sys_lwcond_signal((sys_lwcond_t*) ptr_cond);
}

inline int sys_cond_broadcast(void* ptr_cond)
{
	return sys_lwcond_signal_all((sys_lwcond_t*) ptr_cond);
}

inline int sys_cond_destroy(void* ptr_cond)
{
	return sys_lwcond_destroy((sys_lwcond_t*) ptr_cond);
}

inline void sys_cond_free(void* ptr_cond)
{
...
inline int sys_thread_create(void* ptr_thread, void (*func)(void*), void* arg)
{
	...
	return sys_ppu_thread_create((sys_ppu_thread_t*) ptr_thread, func, (uint64_t) arg, 1001, 0x2000, SYS_PPU_THREAD_CREATE_JOINABLE, "");
}

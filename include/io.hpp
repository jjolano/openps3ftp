...
namespace FTP
{
	namespace IO
	{
		...
		int32_t readdir(int32_t fd, ftp_dirent* dirent, uint64_t* nread);
		...
		int32_t chmod(std::string path, ftp_mode mode);
		int32_t lseek(int32_t fd, int64_t offset, int32_t whence, uint64_t* pos);
		int32_t mkdir(std::string path, ftp_mode mode);
		int32_t rmdir(std::string path);
		int32_t unlink(std::string path);
		int32_t stat(std::string path, ftp_stat* stat);
	};
};
